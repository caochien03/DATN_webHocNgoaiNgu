...
function buildQuestions(target: CardRow[], pool: CardRow[]): Question[] {
...
function QuizContent() {
  ...
  function choose(opt: string) {
    ...
    recordAttempt(q.card.id, isCorrect);
  }
  ...
  return (
    <div className="mx-auto w-full max-w-lg px-4 py-8">
      ...
      {deck && cards.length < OPTION_COUNT ? (
        <p className="mt-6 text-sm text-zinc-500">
          Cần tối thiểu {OPTION_COUNT} thẻ để luyện trắc nghiệm.
        </p>
      ) : null}

      {deck && questions.length > 0 && !done && q ? (
        <>
