...
function LearnContent() {
  ...
  const weakCount = deck?.cards.filter((c) => c.lastResult === false).length ?? 0;
        ...
        <>
          <div className="mt-4 mb-6">
            ...
            <p className="mt-1.5 text-xs text-muted-foreground">
              Bộ gồm <strong className="text-foreground">{deck.cards.length} thẻ</strong> · Chọn một hình thức để bắt đầu luyện tập:
            </p>
          </div>
          ...
          {weakCount > 0 ? (
            <Link
              ...
              className="group relative mb-4 flex items-center justify-between gap-4 overflow-hidden rounded-3xl border border-amber-500/30 bg-amber-500/10 p-4 sm:p-5 transition-all duration-200 hover:-translate-y-0.5 hover:bg-amber-500/15"
              ...
              <div className="flex items-center gap-4 pl-1">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <p className="text-base font-bold text-amber-500">Ôn tập các từ hay sai</p>
                    <span className="rounded-full bg-amber-500/20 px-2 py-0.2 text-xs font-bold text-amber-500">
                      {weakCount} thẻ
                    </span>
                  </div>
                  <p className="mt-0.5 text-xs text-muted-foreground">
                    Tập trung ôn luyện lại các từ vựng bạn đã trả lời chưa chính xác gần đây.
                  </p>
                </div>
              </div>
              <ChevronRight size={20} className="shrink-0 text-amber-500 transition-transform group-hover:translate-x-1" />
            </Link>
          ) : null}
                  ...
                  {enough ? (
                    ...
                    <div className="flex items-center gap-4 rounded-3xl border border-dashed border-border bg-card/40 p-4 opacity-60">
                      ...
                      <div>
                        ...
                        <p className="mt-0.5 text-xs text-muted-foreground">
                          Cần tối thiểu {m.minCards} thẻ để mở chế độ này.
                        </p>
                      </div>
                    </div>
                  )}
