...
export function WritingGradeView({ answer }: { answer: GradedTopikAnswer }) {
  if (answer.gradeStatus === "ai_graded") {
    ...
    return (
      <div className="mt-2 rounded-lg border border-emerald-500/30 bg-emerald-500/10 p-2.5">
        ...
        {answer.aiFeedback ? (
          <p className="mt-1 text-xs leading-relaxed text-emerald-800 dark:text-emerald-200/80">
            {answer.aiFeedback}
          </p>
        ) : null}
        {answer.writingPartResults?.some((p) => p.aiScore != null) ? (
          <ul className="mt-2 flex flex-col gap-1.5">
            {answer.writingPartResults.map((part) => (
              <li key={part.label} className="text-xs text-emerald-700 dark:text-emerald-100/90">
                <span className="font-medium">{part.label}</span>:{" "}
                {part.aiScore ?? 0}
                {part.maxScore != null ? `/${part.maxScore}` : ""}
                {part.aiFeedback ? (
                  <span className="text-emerald-600 dark:text-emerald-200/70"> — {part.aiFeedback}</span>
                ) : null}
              </li>
            ))}
          </ul>
  ...
  if (writingGradeUiStatus(answer) === "pending") {
    return (
      <div className="mt-2 rounded-lg border border-amber-500/30 bg-amber-500/10 p-2.5 text-xs leading-relaxed text-amber-800 dark:text-amber-200">
    ...
    return (
      <div className="mt-2 space-y-1 text-sm">
        <p className="font-semibold text-emerald-700 dark:text-emerald-300">
          ...
          <span className="ml-1 font-normal text-emerald-600 dark:text-emerald-400/80">
